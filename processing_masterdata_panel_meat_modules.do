...
foreach i in 2003 2004 2005 2006 2007 2008 2009 2010 2011 2012 2013 2014{
	...
	* Generating County level prices to be quantity adjusted 
	...
	drop dup
	by fips_code purchase_month module, sort: egen EXP = total(total_price_paid)
	by fips_code purchase_month module, sort: egen Qc = total(wqc)
	by fips_code purchase_month module, sort: gen P = EXP/Qc
	...
	* Generating household level price adjusted quantities
	...
	replace wq = . if dup > 1
	by household_code purchase_month module, sort: egen Q = total(wq)
	drop dup

	* Generating expenditures for each households (Expenditure in module and total expenditure over all modules)
	* Subscrpit i denotes product modules, h denotes for each household
	by household_code purchase_month module, sort: egen expenditure_h_i = total(total_price_paid)
	by household_code purchase_month, sort: egen total_expenditure = total(total_price_paid)

	* Generating budget shares for each product module purchased by households in a month
	by household_code purchase_month module, sort: gen bs = expenditure_h_i/total_expenditure
	
	* Generating Stone's Price index for LAIDS
	gen ln_p = ln(P)
	by household_code purchase_month module, sort: gen bslnp = bs*ln_p
	by household_code purchase_month module, sort: gen dup = cond(_N == 1, 0,_n)
	...
	*by household_code purchase_month module, sort: egen EXP_h = total(exp_h)
	*by fips_code purchase_month module, sort: gen P = EXP_h/Q
	...
	* Reshaping variables that vary over time, product types, and household into panel forms
	...
	keep household_code monthly_date P Q module bs
	collapse(first) P Q bs, by(household_code monthly_date module)
	reshape wide P Q bs, i(household_code monthly_date) j(module)
	...
	tab _merge
	drop _merge
	save  "C:\Users\cmlvh\OneDrive\桌面\Dissertation - Food data\Processed Data\panelmodule_household_`i'.dta",replace	
}

********************************************************************************
* Appending yearly panels into panel data set for analysis
********************************************************************************
use  "C:\Users\cmlvh\OneDrive\桌面\Dissertation - Food data\Processed Data\panelmodule_household_2004.dta"
foreach i in 2003 2005 2006 2007 2008 2009 2010 2011 2012 2013 2014{
	tab monthly_date
	append using  "C:\Users\cmlvh\OneDrive\桌面\Dissertation - Food data\Processed Data\panelmodule_household_`i'.dta"
	tab monthly_date
	save  "C:\Users\cmlvh\OneDrive\桌面\Dissertation - Food data\Processed Data\panelmodule_household_level_processed.dta", replace
}
